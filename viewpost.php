...
if(isset($_POST['SendReport']) ){
    if(isset($_POST['Reason'])){
     ...
     $getReportCount = "SELECT * FROM amx_post_tbl WHERE post_id = '$postID'";
     ...
     if (!$conn->query($sqlReport)) {
         echo $conn->error;
             ...
             $getResult = mysqli_query($conn, $getReportCount);
             $row  = mysqli_fetch_assoc($getResult);
             $before = $row['report_amount'];
             $now = $before + 1;
             $UpdateReportCount = "UPDATE amx_post_tbl SET report_amount = '$now' WHERE post_id = '$postID'";
             if (!$conn->query($UpdateReportCount)) {
                 echo $conn->error;
             }
             else{
